...
namespace FixMath.NET
{
  ...
  public partial struct Fix64 //: IEquatable<Fix64>, IComparable<Fix64> 
  {
    ...
    public static readonly fp MaxValue = new(MAX_VALUE);
    public static readonly fp MinValue = new(MIN_VALUE);
    ...
    //const long LN2 = 0xB17217F7;       // uint32 Q32.32 --   2,977,044,471 / 2^32 = 0.6931471803691238
    const long LN2 = 0xB172;             // ushort Q48.16 --           45426 / 2^16 = 0.693145751953125

    //const long LOG2MAX = 0x1F00000000;  // int64 Q32.32 -- 133,143,986,176 / 2^32 = 31
    const long LOG2MAX = 0x2F0000;        // int32 Q48.16 --       3,080,192 / 2^16 = 47

    //const long LOG2MIN = -0x2000000000; // int64 Q32.32 -- 137,438,953,472 / 2^32 = 32
    const long LOG2MIN = -0x100000;       // int32 Q48.16 --       1,048,576 / 2^16 = 16
    ...
    public static fp Abs(fp value)
    {
      ...
      return new fp((value.value + mask) ^ mask);
    }
    ...
    public static fp Ln(fp x) { return Log2(x) * Ln2; } 
    ...
    public static fp Pow2(fp x)
    {
      ...
      if (x == F64.C1)
        return neg ? F64.C1 / F64.C2 : F64.C2;
      if (x >= Log2Max) return neg ? F64.C1 / MaxValue : MaxValue;
      if (x <= Log2Min) return neg ? MaxValue : F64.C0;
    ...
    //  fp result = fixmath.Atan(fixmath.Sqrt(F64.C1 - x * x) / x);
    //  if (x.value < 0)
    //    return result + Pi;
    //  else
    //    return result;
    ...
    /// </summary>
    public static fp FractionalPart(fp value) { return FromRaw(value.value & 0x000000000000FFFFL); }

    public static fp Round(fp value)
    {
      var fractionalPart = value.value & 0x000000000000FFFF;
      var integralPart = Floor(value);
      if (fractionalPart < 0x8000)
      {
        return integralPart;
      }
      if (fractionalPart > 0x8000)
      {
        return integralPart + F64.C1;
      }
    ...
    //{
    //  int result = 0;
    //  while ((x & (short)0xF000000000000000) == 0) { result += 4; x <<= 4; }
    ...
    //                        tan = (double)MaxValue;
    //                    }
    //                    var rawValue = (((decimal)tan > (decimal)MaxValue || tan < 0.0) ? MaxValue : (fp)tan).value;
    //                    writer.Write(string.Format("0x{0:X}L, ", rawValue));
    //                }
    //                writer.Write(
    //@"
