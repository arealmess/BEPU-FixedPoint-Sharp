...
namespace BEPUphysics.Character
{
  ...
  {
    private static RawList<CharacterContact> supportContacts = new();
    private static RawList<CharacterContact> tractionContacts = new();
    private static RawList<CharacterContact> sideContacts = new();
    private static RawList<CharacterContact> headContacts = new();

    // Debug UI
    public static CharacterContact[][] GetContacts()
    {
      CharacterContact[][] contactLists = new CharacterContact[4][];

      contactLists[0] = supportContacts.ToArray();
      contactLists[1] = tractionContacts.ToArray();
      contactLists[2] = sideContacts.ToArray();
      contactLists[3] = headContacts.ToArray();
    ...
    private void UpdateSupportData(ref Vector3 down)
    {
      ...
      if (contacts.Count > 1)
      {
        ...
        {
          Vector3.Multiply(ref supportData.Normal, F64.C1 / fixmath.Sqrt(length), out supportData.Normal);
        }
      }
    ...
    bool TryDownCast(ref Ray ray, fp length, out bool hasTraction, out SupportRayData supportRayData)
    {
      ...
      hasTraction = false;
      if (QueryManager.RayCast(ray, length, out RayHit earliestHit, out Collidable earliestHitObject))
      {
        ...
        Vector3.Divide(ref earliestHit.Normal, fixmath.Sqrt(lengthSquared), out earliestHit.Normal);
        //A collidable was hit!  It's a support, but does it provide traction?
        earliestHit.Normal.Normalize();
        Vector3.Dot(ref ray.Direction, ref earliestHit.Normal, out fp dot);
        if (dot < F64.C0)
        {
