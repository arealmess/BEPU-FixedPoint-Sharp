...
namespace BEPUphysics.Character
{
  ...
  {
    private static fp tractionThreshold;
    private static fp supportThreshold;
    private static fp headThreshold;
     
    // Debug UI
    public static fp[] GetThresholds()
    {
      var thresholds = new fp[3];
      thresholds[0] = tractionThreshold;
      thresholds[1] = supportThreshold;
      thresholds[2] = headThreshold; 
      return thresholds;
    }
    ...
    public fp TractionThreshold
    {
      ...
      set
      {
        if (value < Toolbox.BigEpsilon || value > F64.C1)
          throw new ArgumentException(value.value.ToString(), "Traction threshold values must range from " + Toolbox.BigEpsilon.ToString() + " to " + F64.C1.ToString() + " inclusive.");
    ...
    public fp SupportThreshold
    {
      ...
      {
        if (value < Toolbox.BigEpsilon || value > F64.C1)
          throw new ArgumentException(value.value.ToString(), "Support threshold values must range from " + Toolbox.BigEpsilon.ToString() + " to 1, inclusive.");
    ...
    public fp HeadThreshold
    {
      ...
      set
      {
        if (value < -1 || value > -Toolbox.BigEpsilon)
          throw new ArgumentException(value.ToString(), "Head threshold values must range from -1 to " + -Toolbox.BigEpsilon + ", inclusive.");
        headThreshold = value;
      }
    }
    ...
    {
      get { return fixmath.Acos(TractionThreshold); }
      set { TractionThreshold = fixmath.Cos(value); }
    }
    /// <summary>
    ...
    {
      get { return fixmath.Acos(SupportThreshold); }
      set { SupportThreshold = fixmath.Cos(value); }
    }
                                                        ...
                                                        where TOutputContacts : IList<CharacterContact>
    {
      ...
      for (int i = pair.ContactCount - 1; i >= 0; --i)
      {
        ...
        Vector3.Subtract(ref characterContact.Contact.Position, ref characterCollidable.worldTransform.Position, out Vector3 offset);
        Vector3.Dot(ref characterContact.Contact.Normal, ref offset, out fp dot);

        Godot.GD.Print("contact dot: " + dot.ToString());
