...
namespace BEPUutilities
{
  ...
  public struct Quaternion : IEquatable<Quaternion>
  {
    ...
    public static void Normalize(ref Quaternion quaternion, out Quaternion toReturn)
    {
      ...
      toReturn = fixmath.Normalize(quaternion);
    }
    ...
    public static void GetAxisAngleFromQuaternion(ref Quaternion q, out Vector3 axis, out fp angle)
    {
      ...
      if (lengthSquared > F64.C1em14)
      {
        ...
        angle = F64.C2 * fixmath.Acos(fixmath.Clamp(qw, -1, F64.C1));
      }
      else
      {
    ...
    {
      //fp dot = fixmath.Dot(v1, v2);
      Vector3.Dot(ref v1, ref v2, out fp dot);
      ...
      if (dot < F64.Cm0p9999) //parallel, opposing direction
      {
        ...
        if (absX < absY && absX < absZ)
          q = new fp4(F64.C0, -v1.Z, v1.Y, F64.C0);
        else if (absY < absZ)
          q = new fp4(-v1.Z, F64.C0, v1.X, F64.C0);
        else
          q = new fp4(-v1.Y, v1.X, F64.C0, F64.C0);
      }
      else
      {
        //fp3 axis = fixmath.Cross(v1, v2);
        Vector3.Cross(ref v1, ref v2, out Vector3 axis); 
        q = new fp4(axis.X, axis.Y, axis.Z, dot + F64.C1);
      }
      //fixmath.Normalize((fp4)q);
      q.Normalize();
    }
