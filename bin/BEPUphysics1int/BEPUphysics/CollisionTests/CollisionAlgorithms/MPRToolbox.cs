    ...
    public static class MPRToolbox
    {
        ...
        public static void LocalSurfaceCast(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref Vector3 direction, out fp t, out Vector3 normal)
        {
            ...
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                ...
                if (rayLengthSquared > Toolbox.Epsilon * F64.C0p01)
                    Vector3.Divide(ref direction, fixmath.Sqrt(rayLengthSquared), out normal);
                else
                    normal = new Vector3();
            ...
            while (true)
            {
                ...
                {
                    //normal = n;
                    //fp normalLengthInverse = 1 / normal.Length();
                    ...
                    {
                        Vector3.Divide(ref n, fixmath.Sqrt(lengthSquared), out normal);
        ...
        public static bool GetContact(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB, ref Vector3 penetrationAxis, out ContactData contact)
        {
            ...
            if (MPRToolbox.AreLocalShapesOverlapping(shapeA, shapeB, ref localTransformB))
            {
                ...
                {
                    Vector3.Divide(ref penetrationAxis, fixmath.Sqrt(lengthSquared), out rayCastDirection);// (Vector3.Normalize(localDirection) + Vector3.Normalize(collidableB.worldTransform.Position - collidableA.worldTransform.Position)) / 2;
                ...
                if (lengthSquared > Toolbox.Epsilon)
                {
                    Vector3.Divide(ref localTransformB.Position, fixmath.Sqrt(lengthSquared), out rayCastDirection);
        ...
        public static bool Sweep(ConvexShape shapeA, ConvexShape shapeB, ref Vector3 sweepA, ref Vector3 sweepB, ref RigidTransform transformA, ref RigidTransform transformB, out RayHit hit)
        {
            ...
            if (rayLengthSquared > Toolbox.Epsilon * F64.C0p01)
            {
                ...
                sweepLength += (shapeA.MaximumRadius + shapeB.MaximumRadius) / fixmath.Sqrt(rayLengthSquared);
            }
            else
            {
        ...
        private static bool LocalSweepCast(ConvexShape shape, ConvexShape shapeB, fp sweepLength, fp rayLengthSquared, ref Vector3 localDirection, ref Vector3 sweep, ref RigidTransform localTransformB, out RayHit hit)
        {
            ...
            if (n.LengthSquared() < Toolbox.Epsilon * F64.C0p01)
            {
                ...
                if (rayLengthSquared > Toolbox.Epsilon * F64.C0p01)
                    Vector3.Divide(ref localDirection, fixmath.Sqrt(rayLengthSquared), out hit.Normal);
                else
                    hit.Normal = new Vector3();
            ...
            while (true)
            {
                ...
                if (supportDot - dot < rayCastSurfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    {
                        Vector3.Divide(ref n, fixmath.Sqrt(lengthSquared), out hit.Normal);
        ...
        internal static void GetLocalPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref Vector3 minkowskiPosition, out Vector3 position)
        {
            ...
            while (true)
            {
                ...
                //    {
                //        fp inverseTotalVolume = 1 / v0v1v2v3volume;
                //        fp v0Weight = ov1v2v3volume * inverseTotalVolume;
                //        fp v1Weight = v0ov2v3volume * inverseTotalVolume;
                //        fp v2Weight = v0v1ov3volume * inverseTotalVolume;
                //        fp v3Weight = 1 - v0Weight - v1Weight - v2Weight;
                //        position = v1Weight * v1A + v2Weight * v2A + v3Weight * v3A;
                //    }
                //    else
                //    {
        ...
        public static bool RayCast(Ray ray, fp maximumLength, ConvexShape shape, ref RigidTransform transform, out RayHit hit)
        {
            ...
            if (rayLengthSquared > Toolbox.Epsilon * F64.C0p01)
            {
                ...
                sweepLength /= rayLengthSquared;
                //Scale the sweep length by the margins.  Divide by the length to pull the margin into terms of the length of the ray.
                sweepLength += shape.MaximumRadius / fixmath.Sqrt(rayLengthSquared);
            }
            else
            {
                rayLengthSquared = F64.C0;
